...
def modify_rating_scale(df, dataset):
    df_mod = df.copy()
    ...
    if dataset == "helpsteer3":
        ...
        df_mod['score'] = df_mod['score'].apply(modify_score)
    
    if dataset == "helpsteer2":
        def modify_dimension(score):
            if score in [1, 2]:
                return 0
            if score in [3, 4, 5]:
                return 1
        ...
        dimension_cols = [f'feat_{dimension}' for dimension in datasets['helpsteer2']]
        for column in dimension_cols:
            df_mod[column] = df_mod[column].apply(modify_dimension)
...
if __name__ == "__main__":
    ...
    for dataset in datasets.keys():
        ...
        X_train_aug = train_merged.drop(columns=[c for c in train_merged.columns if c in metadata_cols]).select_dtypes(include=[np.number]).values
        X_test_aug = test_merged.drop(columns=[c for c in test_merged.columns if c in metadata_cols]).select_dtypes(include=[np.number]).values

        accuracy, f1 = train_and_evaluate(X_train_aug, y_train, X_test_aug, y_test)

        train_modified = modify_rating_scale(train_merged, dataset)
        test_modified = modify_rating_scale(test_merged, dataset)

        X_train_mod = train_modified.drop(columns=[c for c in train_modified.columns if c in metadata_cols]).select_dtypes(include=[np.number]).values
        X_test_mod = test_modified.drop(columns=[c for c in test_modified.columns if c in metadata_cols]).select_dtypes(include=[np.number]).values

        accuracyModified, f1Modified = train_and_evaluate(X_train_mod, y_train, X_test_mod, y_test)

        accuracy_scores.append(accuracy)
        f1_scores.append(f1)

        accuracyModified_scores.append(accuracyModified)
        f1Modified_scores.append(f1Modified)

        print(f"Dataset: {dataset}")
        print(f"Accuracy: {accuracy:.4f}")
        print(f"F1 Score: {f1:.4f}")
        print(f"Modified Accuracy: {accuracyModified:.4f}")
        print(f"Modified F1 Score: {f1Modified:.4f}")
        print("-" * 30)
    
    print("Overall Performance:")
    print(f"Average Accuracy: {np.mean(accuracy_scores)}")
    print(f"Average F1 Score: {np.mean(f1_scores)}")
    print(f"Average Modified Accuracy: {np.mean(accuracyModified_scores)}")
    print(f"Average Modified F1 Score: {np.mean(f1Modified_scores)}")